...
If MULTIPLE sheets present the SAME property's units in different formats (a formatted view, a pivot/table view, a raw-data tab, or a manually "extracted"/normalized copy), mark ONLY the most authoritative and complete ONE as unit-level (prefer the original/source view over derived copies) and mark the others false. When choosing between duplicate views, prefer the sheet whose rows carry ACTUAL current rents — tenant-paid and/or subsidy amounts that vary by unit — over views showing only scheduled/market/zeroed rent figures. Also inspect each duplicate view for internal corruption before trusting it: derived tabs are often built with filled-down formulas that reference the wrong column, so a descriptive column (unit type, BD/BA, bedrooms, square feet) whose values are rent-sized currency amounts duplicating another column means that view is CORRUPTED — mark it false and pick the clean view instead, even when the corrupted view is the better-formatted one. This corruption check works in ONE direction only — it disqualifies a view whose columns contradict their own headers; it does NOT make tidiness a virtue: a messy raw pasted report whose per-unit values are consistent with their headers still beats a tidy derived/normalized copy of it. A view carrying review/QA columns (confidence scores, extraction notes, reviewer comments) is a derived copy of some other view — mark it false. Different sheets covering DIFFERENT buildings/properties should all be marked true.
